...
| 60c | Ретрай ухудшает rate limit | DOCUMENT | Автоматический ретрай отсутствует by design. komparu никогда не повторяет неудачные запросы — пользователь обрабатывает ретраи на своём уровне. |
