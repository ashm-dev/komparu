...
> **komparu** \[компару\] — повелительная форма глагола *kompari* (сравнивать) на эсперанто. Буквально: «сравни!» Вся суть библиотеки — в одном слове.
...
- **Quick check** — выборочная проверка до 5 ключевых смещений (начало, конец, 25%, 50%, 75%) перед полным сканированием (ловит большинство различий за O(1))
...
- **Хеш-сравнение архивов** — `hash_compare=True` для O(entries) по памяти через потоковый FNV-1a 128-бит
- **Пулинг соединений** — CURLSH общий DNS/connection/TLS кеш для всех HTTP-запросов
...
komparu.configure(
    ...
    quick_check=True,          # выборочная проверка ключевых смещений
...
С версии 0.1.0 quick_check проверяет смещение 25%, поэтому это различие теперь обнаруживается за O(1). Бенчмарки ниже записаны со старым 3-точечным quick_check и показывают производительность последовательного mmap+memcmp сканирования.
...
**quick_check** в komparu проверяет до 5 ключевых смещений до полного сканирования. Это позволяет мгновенно обнаружить различие в конце файла (**30us для 1GB**), пока все конкуренты вынуждены прочитать файл целиком. Плоская красная линия — O(1) вне зависимости от размера файла.
